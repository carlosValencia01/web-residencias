...
export class ListGraduatesPageComponent implements OnInit {
 ...
 // Generar reporte de alumnos
  generateReport() {
    ...
    const header = 'Reporte Alumnos Graduados ' + this.searchCarreer;
    ...
    const footer = '© ITT Instituto Tecnológico de Tepic\nTepic, Nayarit, México \n';
    doc.setTextColor(0, 0, 0);
    doc.setFontStyle('bold');
    doc.setFontSize(7);
    ...
    // Hour PDF
    const hour = today.getDate() + '/' + mes + '/' + today.getFullYear() + ' - ' +
  ...
  generateBallotPaper() {
    if (this.alumnosBallotPaper.length !== 0) {
      ...
      const divAlumnosBallotPaper = [];

      // 4 Alumnos por hoja
      const LONGITUD_PEDAZOS = 4;
      for (let i = 0; i < this.alumnosBallotPaper.length; i += LONGITUD_PEDAZOS) {
        const pedazo = this.alumnosBallotPaper.slice(i, i + LONGITUD_PEDAZOS);
  ...
  generateReportBestAverage() {
    ...
    const header = 'Reporte Alumnos Mejor Promedio';
    doc.setTextColor(0, 0, 0);
    doc.setFontSize(15);
    doc.setFontStyle('bold');
    doc.text(header, pageWidth / 2, 30 , 'center');
