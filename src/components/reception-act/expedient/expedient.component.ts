...
export class ExpedientDocumentsComponent implements OnInit {
  ...
  public changeDocument: boolean = false;  
  ...
  init() {
    ...
    this.registeredDate = moment(new Date(this.Request.applicationDate ? this.Request.applicationDate : this.Request.applicationDateLocal)).format('LL');
    ...
    this._StudentProvider.getStudentById(this.Request.studentId).subscribe(
      async (st)=>{        
          this.photo = st.student[0].documents.filter( doc=> doc.type == 'DRIVE').filter( doc=> doc.filename.toLowerCase().indexOf('foto') !== -1)[0];          
          if(this.photo){
