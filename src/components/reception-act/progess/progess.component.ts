...
export class ProgessComponent implements OnInit {
    ...
    this.requestProvider.getAllRequestByStatus(filter).subscribe(
      res => {
        ...
        res.request.forEach(element => {
          const isProcess = this.tabNumber == 3 ? element.documents.filter( (doc)=> doc.status == 'Process').length > 0 : true; //for tab dictamination
          if(isProcess){
  ...
  refresh() {
    ...
    // }
    // this.requestFilter = this.filter(this.careers, this.tabPhases).slice(0);
    if(this.tabNumber == 2){
