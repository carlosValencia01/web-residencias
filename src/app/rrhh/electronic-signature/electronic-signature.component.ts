...
export class ElectronicSignatureComponent implements OnInit {
  ...
  getDocument() {
    this.eSignatureProvider.getDocument(this.employee.employee._id, this.currentPosition._id).subscribe(data => {
      this._downloadSignatureFile(data.text());
    });
  }
  ...
  changePassword() {
    if (this.isMatchPasswordsChange) {
      this.eSignatureProvider.changeESignaturePassword(this.user._id, this.employee.employee._id, this.currentPosition._id, {
      ...
      }).subscribe((res: { doc: string, message: string }) => {
        this.formGroupPswChange.reset();
        this.notification
          .showNotification(eNotificationType.SUCCESS, 'Firma electrónica', res.message || 'Contraseña actualizada correctamente');
        if (res.doc) {
          this._downloadSignatureFile(res.doc);
        } else {
          this.notification
        ...
        const err_msj = JSON.parse(err && err._body || '').message || '';
  ...
  private _downloadSignatureFile(data: string): void {
    if (window.navigator && window.navigator.msSaveOrOpenBlob) {
      window.navigator.msSaveOrOpenBlob(data, `${this.fileName}.itt`);
    } else {
