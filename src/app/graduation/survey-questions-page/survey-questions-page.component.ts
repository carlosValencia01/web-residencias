...
export class SurveyQuestionsPageComponent implements OnInit {
  ...
  {
    this.idDocAlumn=this.router.url.split('/')[3];
    ...
    let sub = firestoreService.getProfile(this.idDocAlumn).subscribe(
      profile=>{
        ...
        if(!isNullOrUndefined(profile)){   
          ...
          if(this.profileItem && !this.profileItem.survey){
            ...
            Swal.fire({
              ...
              if (result.value) {
                window.location.assign("/graduation/my-graduation") //salir de la encuesta
              }
            }) 
          }                         
        }else{
            ...
            res => {
              ...
              if(data !== undefined){ // Verificar que existan datos de alumno en evento principal
                ...
                Swal.fire({
                  ...
                  if (result.value) {
                    window.location.assign("/graduation/surveyFind") //salir de la encuesta
                  }
                })
              }
            }
